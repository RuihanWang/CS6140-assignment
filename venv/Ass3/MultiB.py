...
class MultiB:
    ...
    def computeData(self, trainData,trainlabel):
        ...
        return df,PriorP,words,totalDocs
    ...
    def train(self, df,totalDocs):
        # Total words need to be adjusted to vocabalury
        pwordclass = df.groupby(['classIdx', 'wordIdx'])
        pclass = df.groupby(['classIdx'])
        PrMN = (pwordclass['count'].count() + 1) / (pclass['count'].count() + totalDocs)
        PrMN = PrMN.unstack()
        for c in range(1, 21):
            PrMN.loc[c, :] = PrMN.loc[c, :].fillna(1 / (pclass['count'].count()[c] + totalDocs))
        PrMN = np.log(PrMN)
                ...
                for wordIdx in newDict[docIdx]:
                    if(wordIdx in words.index):
                        ...
                        try:
                            ...
                            scoreDict[classIdx] += probability
                        except:
                            scoreDict[classIdx] += 0
                            # f*log(Pr(i|j))
    ...
    def foldup(self, trainData, trainLabel, testData, testLabel):
        ...
        p = []
        r = []
        df,priorp,words,totalDocs = self.computeData(trainData,trainLabel)
